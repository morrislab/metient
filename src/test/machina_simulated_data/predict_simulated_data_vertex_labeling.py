...
def fprint(s):
    '''
    Prints without buffering, so output txt file gets updated as trees are run
    '''
    print(s, flush=True)

def predict_vertex_labelings(machina_sims_data_dir, site, mig_type, seed, out_dir, weights, batch_size, weight_init_primary, lr_sched):
    ...
    for adj_matrix, pruned_cluster_label_to_idx in data:
        ...
        G = data_util.get_genetic_distance_tensor_from_adj_matrix(T, pruned_cluster_label_to_idx, ";")
        print_config = vert_util.PrintConfig(visualize=False, verbose=False, viz_intermeds=False)
        ...
        fprint("Number of seeds run:", len(results))
...
if __name__=="__main__":
    ...
    parser.add_argument('--lr_sched', type=str, help="Learning rate schedule. See vertex_labeling.gumbel_softmax_optimization for details", default="constant")
    ...
    fprint(f"Learning rate schedule: {lr_sched}")
    fprint(f"Placing higher weight on primary vertex labeling for all internal nodes: {args.primary_weight}")

    start_time = datetime.datetime.now()
    fprint(f"Start time: {start_time}")
    fprint(f"Using {args.cores} cores.")
    ...
    fprint(f"Number of tasks: {len(futures)}")
    ...
    fprint(f"Finished running {len(results)} simulations.")
    fprint(f"End time: {end_time}. Time elapsed: {end_time - start_time}")
